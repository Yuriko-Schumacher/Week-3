...
let flightsData,
	...
	arrivalBrush,
	filters = {},
	dispatch = d3.dispatch("update"); // what kind of signals that can send or receive?
...
Promise.all([d3.csv("data/flights-filtered.csv")]).then(function (datasets) {
	...
	dispatch.call("update"); // "update" is arbitrary but it should be the same everywhere
	// when we make different files, you can just access to the "update" key
...
function updateData() {
	...
	// dispatch.call("update"); NEVER DO THAT!!! INFINITE LOOP!!!!
}
...
function drawDelayChart(data = flightsData) {
	...
	if (!delayScaleX) {
		delayScaleX = d3.scaleLinear().domain([0, 150]).range([0, size.w]);
	}
...
function loadTable(data = flightsData) {
	let flightRowSel = d3
		.select("div.flight-table-content")
	...
	flightRowSel
		.selectAll("div.flight-destination")
		.data((d) => [d])
		.join("div")
